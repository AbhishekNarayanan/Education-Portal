...
class Comments {
  ...
  function get($pid = 0) {
    ...
    $sql = "SELECT * FROM `comments` WHERE `post_id`=? AND `video_id`=? ORDER BY `reply_id`";
    $this->stmt = $this->pdo->prepare($sql);
    $this->stmt->execute([$pid,$vid]);
    $comments = [];
    while ($row = $this->stmt->fetch(PDO::FETCH_NAMED)) {
  ...
  //       $rid : reply id
    ...
    $fields = "`post_id`, `name`, `message`, `video_id`";
    $values = "?, ?, ?, ?";
    $vid=$_COOKIE['vid'];
